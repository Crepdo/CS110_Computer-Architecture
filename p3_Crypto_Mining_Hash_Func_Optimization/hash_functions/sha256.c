...
/* SIMD */
static inline __m256i ROTR_SIMD(__m256i x, int n)
{
  return _mm256_srli_epi32(x, n) | _mm256_slli_epi32(x, 32 - n);
}

static inline __m256i SHR_SIMD(__m256i x, int n)
{
...
#define ADD4_SIMD(x0, x1, x2, x3) _mm256_add_epi32(_mm256_add_epi32(_mm256_add_epi32(x0, x1), x2), x3)
...
void store_epi32(__m256i x, WORD *x0, WORD *x1, WORD *x2, WORD *x3, WORD *x4, WORD *x5, WORD *x6, WORD *x7)
{
  union
  {
    WORD data[8];
...
void sha256_compress(SHA256_CTX *ctx, WORD *message)
{
  ...
  X0 = _mm256_set_epi32(ctx->state[0], ctx->state[1], ctx->state[2], ctx->state[3], ctx->state[4], ctx->state[5], ctx->state[6], ctx->state[7]);
  X1 = _mm256_set_epi32(a, b, c, d, e, f, g, h);
  X0 = _mm256_add_epi32(X0, X1);
  store_epi32(X0, &ctx->state[0], &ctx->state[1], &ctx->state[2], &ctx->state[3], &ctx->state[4], &ctx->state[5], &ctx->state[6], &ctx->state[7]);
}
...
void sha256_transform_optimized_simd(SHA256_CTX *ctx)
{
  ...
  for (i = 0, j = 0; i < 16; ++i, j += 4)
  {
    M[i] = _mm256_set_epi32(
        ...
        (ctx->data[j + 256] << 24) | (ctx->data[j + 256 + 1] << 16) | (ctx->data[j + 256 + 2] << 8) | (ctx->data[j + 256 + 3]),
        (ctx->data[j + 320] << 24) | (ctx->data[j + 320 + 1] << 16) | (ctx->data[j + 320 + 2] << 8) | (ctx->data[j + 320 + 3]),
        (ctx->data[j + 384] << 24) | (ctx->data[j + 384 + 1] << 16) | (ctx->data[j + 384 + 2] << 8) | (ctx->data[j + 384 + 3]),
        (ctx->data[j + 448] << 24) | (ctx->data[j + 448 + 1] << 16) | (ctx->data[j + 448 + 2] << 8) | (ctx->data[j + 448 + 3]));
  }
  ...
  for (i = 0; i < 64; ++i)
  {
    store_epi32(M[i], &m[0][i], &m[1][i], &m[2][i], &m[3][i], &m[4][i], &m[5][i], &m[6][i], &m[7][i]);
  }
